...
// Make React available to BetweenRaces (simple approach)
window.React = React
...
export default function App() {
  ...
  const isLive = raceState?.is_live || (raceState?.drivers?.length > 0 && raceState?.mode !== 'between_races')
  const isBetween = raceState?.mode === 'between_races' || (!raceState?.is_live && !raceState?.drivers?.length)
            ...
            {isLive ? (
              <>
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.9)', letterSpacing:'0.8px', textTransform:'uppercase', fontWeight:600 }}>
                  {raceState?.circuit_name || 'Live Race'} · Rd {status?.season_round}
                </div>
              ...
              <>
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.9)', letterSpacing:'0.8px', textTransform:'uppercase', fontWeight:600 }}>
                  F1 2026 Season · {status?.season_round || 0}/{status?.total_rounds || 22} Races
                </div>
                <div style={{ fontSize:10, color:'rgba(255,255,255,0.55)' }}>
                  {nextRace ? `Next: ${nextRace.name?.replace(' Grand Prix',' GP')} in ${raceState?.days_until_next} days` : 'Season complete'}
                </div>
              </>
            )}
          </div>
      ...
      {isLive && (
        <div style={{ background:'#141414', padding:'4px 12px 3px', flexShrink:0 }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', marginTop:2, fontSize:9, fontFamily:"'Share Tech Mono'", color:'rgba(255,255,255,0.25)' }}>
            ...
            <span>{Math.round((raceState.current_lap/raceState.total_laps)*100)}% COMPLETE</span>
            <span>{raceState.total_laps - raceState.current_lap} LEFT</span>
          </div>
        </div>
      )}

      {/* Tabs — only show when live */}
      {isLive && (
