...
function TrackDetail({race, onClose}) {
  ...
  const cfg = CIRCUITS[race.circuit] || DEFAULT_CIRCUIT
      ...
      <div style={{maxWidth:560,margin:'0 auto',padding:'0 0 40px'}}>
        ...
        <div style={{background:'#111',padding:'16px',display:'flex',justifyContent:'center',borderBottom:'1px solid rgba(255,255,255,0.07)'}}>
          <svg width="100%" viewBox={cfg.viewBox} style={{maxWidth:320}}>
            <path d={cfg.path} fill="none" stroke="#111" strokeWidth={11} strokeLinecap="round" strokeLinejoin="round"/>
            <path d={cfg.path} fill="none" stroke="#2e2e2e" strokeWidth={9} strokeLinecap="round" strokeLinejoin="round"/>
            {(cfg.sectors||[]).map((s,i)=>(
              <path key={i} d={s.d} fill="none" stroke={s.color} strokeWidth={3} opacity={0.6} strokeLinecap="round"/>
            ))}
            {cfg.sf && <line x1={cfg.sf.x1} y1={cfg.sf.y1} x2={cfg.sf.x2} y2={cfg.sf.y2} stroke="rgba(255,255,255,0.8)" strokeWidth={3}/>}
            {cfg.name_pos && <text x={cfg.name_pos.x} y={cfg.name_pos.y} fontFamily="'Rajdhani'" fontSize={11} fill="rgba(255,255,255,0.12)" textAnchor="middle" fontWeight={700} letterSpacing={3}>{cfg.name}</text>}
          </svg>
        </div>
