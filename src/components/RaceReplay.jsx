...
export default function RaceReplay({ lastRace }) {
  ...
  useEffect(() => {
    ...
    if (fallback) { setSvgPath(fallback.path); setVBox(fallback.viewBox||'0 0 500 500'); return }
    fetchCircuitPath(circuit).then(p => {
      setSvgPath(p || DEFAULT_CIRCUIT.path)
      setVBox('0 0 500 500')
    })
  }, [circuit])
      ...
      {isLoading ? (
        ...
        <div style={{ padding:'8px 4px', display:'flex', justifyContent:'center' }}>
          <svg width="100%" viewBox={vBox} style={{ maxWidth:340 }}>
            ...
            {/* Track layers */}
            <path d={svgPath||""} fill="none" stroke="#000" strokeWidth={13} strokeLinecap="round" strokeLinejoin="round" opacity={0.6}/>
            <path d={svgPath||""} fill="none" stroke="#252525" strokeWidth={9} strokeLinecap="round" strokeLinejoin="round"/>

<line x1="245" y1="350" x2="260" y2="350" stroke="rgba(255,255,255,0.5)" strokeWidth={2}/>
